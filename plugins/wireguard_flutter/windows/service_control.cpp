...
namespace wireguard_flutter
{
  ...
  void ServiceControl::CreateAndStart(CreateArgs args)
  {
    ...
    // Always remove stale service so we recreate with the current config file.
    SC_HANDLE old_service = OpenService(service_manager, &service_name_[0], SERVICE_ALL_ACCESS);
    if (old_service != NULL)
    {
      SERVICE_STATUS svcStatus;
      ControlService(old_service, SERVICE_CONTROL_STOP, &svcStatus);
      // Wait briefly for the service to stop
      for (int i = 0; i < 10; i++)
      {
        SERVICE_STATUS_PROCESS ssp;
        DWORD needed;
        if (QueryServiceStatusEx(old_service, SC_STATUS_PROCESS_INFO,
      ...
      DeleteService(old_service);
      CloseServiceHandle(old_service);
    }

    EmitState("connecting");
    SC_HANDLE service = CreateService(service_manager,                  // SCM database
